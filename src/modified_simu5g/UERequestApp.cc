...
void UERequestApp::initialize(int stage)
{
    ...
    fixedDeadline_ = par("fixedDeadline");
    ...
    taskSuccess_ = registerSignal("taskSuccess");
...
void UERequestApp::handleMessage(cMessage *msg)
{
    ...
    else
    {
        ...
        if (!packet) {
            EV << "UERequestApp: ignoring non-packet message type " << msg->getClassName() << endl;
            delete msg;
            return;
        }
...
void UERequestApp::sendRequest()
{
    ...
    unsigned int taskId = sno_;
    req->setSno(taskId);
    taskDeadlines_[taskId] = simTime() + fixedDeadline_;
    sno_++;
...
void UERequestApp::recvResponse(cMessage* msg)
{
    ...
    unsigned int taskId = res->getSno();

    simtime_t deadline = taskDeadlines_[taskId];

    bool taskSuccess = (respTime <= fixedDeadline_);
    emit(taskSuccess_, taskSuccess ? 1 : 0);
